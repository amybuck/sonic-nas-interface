...
void swp_util_print_tap(swp_util_tap_descr tap) {
    ...
    EV_LOG_TRACE(ev_log_t_INTERFACE, 3, "PKT-IO", "Dev : %s, Queues: %d,",
                     tap->devname,tap->queues);
    for ( ; ix < mx ; ++ix ) {
        EV_LOG_TRACE(ev_log_t_INTERFACE, 3, "PKT-IO", "FD: %d", tap->fds[ix]);
    }
}
...
int swp_util_set_persist(swp_util_tap_descr tap, int how) {
    if ( (tap->fds[0]==SWP_UTIL_INV_FD) ||
        ...
        EV_LOG(ERR,INTERFACE,0,"TAP-PERSIST", "Can't persist tap device %s",
                                tap->devname);
...
t_std_error swp_util_tap_operation(const char * name, int type, bool create) {
    ...
    if ((fd=open("/dev/net/tun",O_RDWR))<0) {
        EV_LOG(ERR,INTERFACE,0,"TAP-PERSIST", "Can't open /dev/net/tun");
    ...
    do {
        ...
        if (err) {
            EV_LOG(ERR,INTERFACE,0,"TAP-OP", "Can't associate tap device %s to FD err:%d",name,
                    err);
            rc = STD_ERR(INTERFACE,FAIL,err);
            break;
        }
        if (ioctl(fd,TUNSETPERSIST,(create) ? 1 : 0)< 0) {
            EV_LOG(ERR,INTERFACE,0,"TAP-OP", "Can't change persist state for %s to %d",
                    name,create);
            rc = STD_ERR(INTERFACE,FAIL,err);
        }
    } while (0);
...
t_std_error swp_util_alloc_tap(swp_util_tap_descr tap, int type) {
    ...
    if (tap==NULL || strlen(tap->devname)>(sizeof(ifr.ifr_ifrn.ifrn_name)-1)) {
    ...
    strncpy(ifr.ifr_ifrn.ifrn_name,tap->devname,
            sizeof(ifr.ifr_ifrn.ifrn_name));

    for ( i = 0; i < tap->queues ; ++i) {
        if ((fd=open("/dev/net/tun",O_RDWR))<0) {
            swp_util_close_fds(tap);
            EV_LOG(ERR,INTERFACE,0,"TAP-ALLOC", "Can't open /dev/net/tun ");
            return STD_ERR(INTERFACE,FAIL,err);
        }

        err = ioctl(fd,TUNSETIFF,(void*)&ifr);
        if (err) {
            EV_LOG(ERR,INTERFACE,0,"TAP-ALLOC", "Can't associate file descriptor for tap (%s:%s) :%d",
