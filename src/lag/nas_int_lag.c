...
t_std_error nas_lag_create(npu_id_t npu_id,ndi_obj_id_t *ndi_lag_id) {
    EV_LOG(INFO,INTERFACE,3,"NAS-LAG","Creating ndi_lag_id in npu %d",
            npu_id);
...
t_std_error nas_del_port_from_lag(npu_id_t npu_id,ndi_obj_id_t ndi_lag_member_id) {
    ...
    EV_LOG(INFO,INTERFACE,3,"NAS-LAG","Deleting NAS Lag member ID  %"PRIu64" ",ndi_lag_member_id);
...
t_std_error nas_set_lag_member_attr(npu_id_t npu_id,ndi_obj_id_t ndi_lag_member_id,
    ...
    EV_LOG(INFO,INTERFACE,3,"NAS-LAG","Block/Unblock NAS port <%d >  %"PRIu64" egress_disable %d "
