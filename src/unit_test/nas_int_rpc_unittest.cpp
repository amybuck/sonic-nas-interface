...
{
    char ifname[64];
    sprintf(ifname, "e101-%03d-0", test_front_panel_port);
...
TEST(int_rpc_test, delete_interface_fanout)
{
    ...
    sprintf(cmdbuf, "os10-config-fanout %s true", ifname);
    ASSERT_TRUE(system(cmdbuf) == 0);
    sprintf(ifname, "e101-%03d-%d", test_front_panel_port, test_subport_id + 1);
    delete_interface_rpc(ifname);
}
...
TEST(int_rpc_test, create_interface_front_panel_fanout)
{
    ...
    sprintf(cmdbuf, "os10-config-fanout %s false", ifname);
    ASSERT_TRUE(system(cmdbuf) == 0);
}
