...
static void test_port_lpbk_mode_set(npu_id_t npu, npu_port_t port, BASE_CMN_LOOPBACK_TYPE_t lpbk_mode)
{
    ...
    cps_api_key_from_attr_with_qual(cps_api_object_key(obj),
                                    BASE_PORT_PHYSICAL_OBJ,
                                    cps_api_qualifier_TARGET);

    cps_api_object_attr_add_u32(obj,BASE_PORT_PHYSICAL_NPU_ID,npu);
    cps_api_object_attr_add_u32(obj,BASE_PORT_PHYSICAL_PORT_ID,port);


    cps_api_object_attr_add_u32(obj, BASE_PORT_PHYSICAL_LOOPBACK, (uint32_t)lpbk_mode);
