...
nas_list_node_t *nas_get_link_node(std_dll_head *p_link_node_list, hal_ifindex_t index)
{
    ...
    while(p_link_node != NULL) {
        ...
        {
            EV_LOG_INFO(ev_log_t_INTERFACE,3, "NAS-LINK", "Found link node %d ",
                               p_link_node->ifindex);
            return p_link_node;
        }
    ...
    if(p_link_node != NULL)
        ...
        EV_LOG_INFO(ev_log_t_INTERFACE,3, "NAS-LINK", "No link node %d found ",
                   index);
...
void nas_delete_port_list(nas_list_t *p_link_node_list)
{
    ...
    if(p_link_node != NULL) {
        EV_LOG_INFO(ev_log_t_INTERFACE, ev_log_s_MINOR, "NAS-Link",
