...
static void nas_process_cps_ports(nas_bridge_t *p_bridge, nas_port_mode_t port_mode,
...
static cps_api_return_code_t nas_vlan_set_admin_status(cps_api_object_t obj, nas_bridge_t *p_bridge)
{
    ...
    {
        EV_LOG(ERR,INTERFACE,ev_log_s_CRITICAL ,"NAS-CPS-VLAN","Failure setting Vlan admin status");
...
static cps_api_return_code_t nas_cps_set_vlan_learning_mode(cps_api_object_t obj, nas_bridge_t *p_bridge)
{
    ...
    if(ndi_set_vlan_learning(0, p_bridge->vlan_id, learning_disable) != STD_ERR_OK) {
        EV_LOG(ERR,INTERFACE,ev_log_s_CRITICAL ,"NAS-Vlan", "Error setting learning disable state for VLAN %d",
               p_bridge->ifindex);
        return cps_api_ret_code_ERR;
    }
...
cps_api_return_code_t nas_cps_set_vlan_mac(cps_api_object_t obj, nas_bridge_t *p_bridge)
{
    ...
    if(nas_os_interface_set_attribute(obj,DELL_IF_IF_INTERFACES_INTERFACE_PHYS_ADDRESS) != STD_ERR_OK) {
        EV_LOG(ERR,INTERFACE,ev_log_s_CRITICAL ,"NAS-Vlan", "Failure setting bridge %d MAC in OS",
               p_bridge->ifindex);
        return cps_api_ret_code_ERR;
    }
    ...
    EV_LOG(INFO,INTERFACE, ev_log_s_CRITICAL ,"NAS-Vlan", "Rcvd MAC  %s set for bridge %d\n",
...
static cps_api_return_code_t nas_cps_update_vlan(nas_bridge_t *p_bridge, cps_api_object_t obj)
{
    ...
    if (tag_list_attr == true) {
        ...
        nas_process_cps_ports(p_bridge, NAS_PORT_TAGGED, tag_port_list);
    }
    if (untag_list_attr == true) {
        ...
        nas_process_cps_ports(p_bridge, NAS_PORT_UNTAGGED, untag_port_list);
    }
    return cps_api_ret_code_OK;
...
static cps_api_return_code_t nas_cps_create_vlan(cps_api_object_t obj)
{
    ...
    if( (vlan_id < MIN_VLAN_ID) || (vlan_id > MAX_VLAN_ID)) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan",
                           "Invalid VLAN ID during create vlan");
        nas_bridge_unlock();
        return rc;
    }
    EV_LOG(INFO, INTERFACE, ev_log_s_MINOR, "NAS-Vlan", "Create VLAN %d using CPS", vlan_id);
    ...
    if (nas_os_add_vlan(obj, &br_index) != STD_ERR_OK) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan",
               "Failure creating VLAN in Kernel");
        nas_bridge_unlock();
        return rc;
    }
...
static cps_api_return_code_t nas_cps_delete_vlan(cps_api_object_t obj)
{
    ...
    EV_LOG(INFO, INTERFACE, ev_log_s_MINOR, "NAS-VLAN-DELETE", "CPS Delete VLAN ");
    cps_api_object_attr_t vlan_name_attr = cps_api_get_key_data(obj, IF_INTERFACES_INTERFACE_NAME);
    ...
    if ((vlan_name_attr == nullptr)  && (vlan_if_attr == nullptr)) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-VLAN-DELETE", "Missing Vlan interface name or if_index");
        return cps_api_ret_code_ERR;
    }
    ...
    if (vlan_if_attr != nullptr) {
        ...
        EV_LOG(INFO, INTERFACE, ev_log_s_MINOR, "NAS-VLAN-DELETE", "CPS Delete VLAN for vlan index %d", br_index);
    } else {
        p_bridge_node  = nas_get_bridge_node_from_name((const char *)cps_api_object_attr_data_bin(vlan_name_attr));
        EV_LOG(INFO, INTERFACE, ev_log_s_MINOR, "NAS-VLAN-DELETE", "CPS Delete VLAN for vlan interface %s",
    ...
    if (p_bridge_node == NULL) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-VLAN-DELETE", "Error finding bridge node");
        nas_bridge_unlock();
        return cps_api_ret_code_ERR;
    }
    ...
    if (nas_os_del_vlan(idx_obj) != STD_ERR_OK) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan",
               "Failure deleting vlan index from kernel");
    }
    ...
    if (nas_cleanup_bridge(p_bridge_node) != STD_ERR_OK) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan", "Failure cleaning vlan data");
...
static cps_api_return_code_t nas_cps_set_vlan(cps_api_object_t obj)
{
    ...
    EV_LOG(INFO, INTERFACE, ev_log_s_MINOR, "NAS-Vlan", "CPS Set VLAN");

    cps_api_object_attr_t vlan_name_attr = cps_api_get_key_data(obj, IF_INTERFACES_INTERFACE_NAME);
    cps_api_object_attr_t vlan_if_attr = cps_api_object_attr_get(obj, DELL_BASE_IF_CMN_IF_INTERFACES_INTERFACE_IF_INDEX);

    if ((vlan_name_attr == nullptr)  && (vlan_if_attr == nullptr)) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan-SET", "Missing Vlan interface name or if_index");
        return cps_api_ret_code_ERR;
    }
                                                      ...
                                                      size_t ix)
{
    ...
    size_t mx = cps_api_object_list_size(param->list);
     ...
     EV_LOG(INFO, INTERFACE, ev_log_s_MAJOR, "NAS-Vlan",
...
t_std_error nas_vlan_lag_register() {
    ...
    EV_LOG(INFO, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan",
               "Registering for LAG event notification!!!");
    ...
    if (cps_api_event_thread_reg(&reg, nas_vlan_lag_event_func_cb, NULL)!=cps_api_ret_code_OK) {
        EV_LOG(ERR, INTERFACE, ev_log_s_CRITICAL, "NAS-Vlan",
                   "Failure registering for LAG event notification!!!");
...
t_std_error nas_cps_vlan_init(cps_api_operation_handle_t handle) {
    ...
    if (intf_obj_handler_registration(obj_INTF, nas_int_type_VLAN,
        ...
        EV_LOG(ERR,INTERFACE, 0,"NAS-VLAN-INIT", "Faild to register VLAN interface CPS handler");
        return STD_ERR(INTERFACE,FAIL,0);
    }
    return nas_vlan_lag_register();
}
                                  ...
                                  nas_port_list_t &port_index_list)
{
    ...
    for (auto it = port_index_list.begin() ; it != port_index_list.end() ; ++it){
        ...
        if(int_type == nas_int_type_PORT) {
            ...
            nas_handle_lag_update_for_vlan(p_bridge, *it, p_bridge->vlan_id,
                                           port_mode, true, true);
        }
    } /* for */
    if (!publish_list.empty()) {
